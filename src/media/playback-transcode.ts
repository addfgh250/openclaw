  ...
  try {
    ...
    const outputBuffer = await withTempWorkspace(
      ...
      async (workspace) => {
        ...
        let inputIdentity: BigIntStats;
        try {
          inputIdentity = await staged.handle.stat({ bigint: true });
        ...
        try {
          ...
          // The move owns its path checks; this binds its result to our completed writer.
          if (
        ...
        const inputPath = input.realPath;
